...
from ClariQDataset import ClariQDatasetDH

from IPython import embed

class AnswerGenDoubleHeadGPT2(pl.LightningModule):
    def __init__(self, hparams):
        super(AnswerGenDoubleHeadGPT2, self).__init__()
        self.hparams.update(vars(hparams))

        # weights for DoubleHead loss
        self.lm_w = 2.0
        self.mc_w = 1.0
        ...
        self.model = GPT2DoubleHeadsModel.from_pretrained(hparams.model_name)
        self.model.resize_token_embeddings(len(self.tokenizer))

    def forward(self, input_ids, attention_mask, token_type_ids, lm_labels, mc_labels):

        outputs = self.model(input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids,
                            labels=lm_labels, mc_labels=mc_labels)
        return outputs


    def configure_optimizers(self):
        optimizer = torch.optim.Adam(self.model.parameters(), lr=self.hparams.lr, correct_bias=True)
        return optimizer

    def train_dataloader(self):
        dataset = ClariQDatasetDH(
        ...
        self.train_dataloader_object = DataLoader(
                                    ...
                                    collate_fn=AnswerGenDoubleHeadGPT2.collate_fn
                                    )
    ...
    def test_dataloader(self):
        ...
        self.test_dataloader_object = DataLoader(
                                    ...
                                    shuffle=False,
                                    num_workers=self.hparams.num_workers, sampler=sampler,
                                    # collate_fn=AnswerGenDoubleHeadGPT2.collate_fn_test
                                    )
        return self.test_dataloader_object

    def training_step(self, batch, batch_nb):
        input_ids, attention_mask, token_type_ids, lm_labels, mc_labels = batch
        ...
        lm_loss = output[0]
        mc_loss = output[1]
        loss = self.lm_w * lm_loss + self.mc_w * mc_loss
        if self.logger:
            self.logger.log_metrics({'train_loss': loss.item(),
                            'train_loss_lm': lm_loss.item(),
                            'train_loss_mc': mc_loss.item()})

        return {'lm_loss': lm_loss, 'mc_loss': mc_loss, 'loss': loss}
    ...
    def validation_epoch_end(self, outputs):
        ...
        avg_loss_lm = torch.stack([x['lm_loss'] for x in outputs]).mean()
        avg_loss_mc = torch.stack([x['mc_loss'] for x in outputs]).mean()

        print(f"\nDEV:: avg-LOSS: {avg_loss} || LOSS_LM: {avg_loss_lm} || LOSS_MC: {avg_loss_mc} ||")

        return {'val_epoch_loss': avg_loss, 'val_epoch_loss_mc': avg_loss_mc, 'val_epoch_loss_lm': avg_loss_lm}
    ...
    def collate_fn(batch):
        ...
        mc_label = torch.stack([x['mc_label'] for x in batch])

        return (input_ids, attention_mask, token_type_ids, lm_label, mc_label)
    ...
    def test_step(self, batch, batch_nb):
        ...
        try:
            history = batch['history'][0].lower()
            # history = batch['history']
        except:
            history = []
        if self.hparams.answer_generation and not self.hparams.history_data:
            question = batch['question'][0].lower()
            out_ids = self.sample_sequence(facets, history, question=question)
        elif self.hparams.history_data:
            question = batch['question'][0]
            query = batch['query'][0]
            out_ids = self.sample_sequence(facets, history, question=question, query=query)
        else:
        ...
        with open('out/' + self.hparams.run_name + '_' + self.hparams.test_ckp.split('=')[1] + '_' + \
            ...
            if self.hparams.answer_generation and not self.hparams.history_data:
                fout.write(facets +'\t'+ str(history) +'\t'+ question +'\t'+ out_test +'\n')
            elif self.hparams.history_data:
                fout.write(facets +'\t'+ query +'\t'+ str(history) +'\t'+ question +'\t'+ out_test +'\n')
            else:
    ...
    def sample_sequence(self, facets, history, current_output=None, question=None, query=None):
        ...
        for i in range(self.hparams.max_output_len):
            if self.hparams.answer_generation and not self.hparams.history_data:
                instance = ClariQDatasetDH.build_input_from_segments(facets, history, question, self.tokenizer, with_eos=False, answer=current_output)
            elif self.hparams.history_data:
                instance = ClariQDatasetDH.build_input_with_history(facets, query, history, question, self.tokenizer, with_eos=False, answer=current_output)
            else:
                instance = ClariQDatasetDH.build_input_from_segments(facets, history, current_output, self.tokenizer,
